...
\begin{frame}{Passo 2: Extração dos Tempos Até a Falha (TTF)}
    \begin{columns}
        ...
        \begin{tabular}{lr@{\hskip 10pt}lr}
            \toprule
            \textbf{Rol.} & \textbf{TTF} & \textbf{Rol.} & \textbf{TTF} \\
            \midrule
            1\_1 & 7,78 & 1\_6 & 5,43 \\
            1\_2 & 2,42 & 1\_7 & 2,08 \\
            2\_1 & 6,59 & 2\_3 & 6,42 \\
...
\begin{frame}{Passo 3: ajuste de distribuições de vida}
    ...
    \normalsize
    \vspace{0.15cm}
    \begin{block}{Como são obtidos (máxima verossimilhança --- MLE)}
        \footnotesize Procuram-se os valores dos parâmetros que \textbf{maximizam a probabilidade} de observar exatamente os 17 TTFs medidos. Ou seja, cada distribuição é ``encaixada'' nos dados reais; os parâmetros saem diretamente dessas 17 vidas.
    \end{block}
\end{frame}
...
\begin{frame}{Passo 3: parâmetros e distribuição vencedora}
    ...
    \begin{block}{A supremacia da Weibull}
        \scriptsize A distribuição \textbf{Weibull} venceu no critério estatístico (menor AICc). Além disso, ela é fundamental na engenharia por oferecer interpretação física direta do parâmetro de forma $\beta$ na curva da banheira. O $\eta$ é o tempo em que $\approx 63\%$ das unidades falham.
    \end{block}
\end{frame}
...
\begin{frame}{Passo 4: intervalos de confiança por \textit{bootstrap}}
    \begin{columns}
        \column{0.5\textwidth}
        \scriptsize
        \textbf{Como é calculado} (repetir $r=10^4$ vezes):
        ...
        \end{center}
        {\tiny Histogramas das $10^4$ reamostras: \textbf{eixo $x$} $=$ valor do parâmetro ($\beta$ ou $\eta$), \textbf{eixo $y$} $=$ frequência. As \textbf{linhas verticais tracejadas} marcam os limites do \textbf{IC 95\%}.}
        \vspace{0.1cm}
        \begin{center}
...
\begin{frame}{Passo 5: $R(t)$ e curva da banheira}
    \begin{columns}
        ...
        \column{0.5\textwidth}
        \begin{center}
            \includegraphics[width=\linewidth]{curva_banheira.png}
        \end{center}
        \begin{block}{Por que só ``desgaste''?}
            \tiny A banheira tem 3 fases: mortalidade infantil ($\beta{<}1$), vida útil ($\beta{\approx}1$) e desgaste ($\beta{>}1$). Aqui há \textbf{um único modo} ($\beta{=}1{,}80$) $\Rightarrow$ \textbf{um único ponto}, na zona de desgaste. Os ensaios partem do rolamento são e o levam direto à \textbf{fadiga} --- não capturam as outras fases. Ter ``dois níveis'' exigiria modos/sub-populações com $\beta$ diferentes.
        \end{block}
    \end{columns}
\end{frame}

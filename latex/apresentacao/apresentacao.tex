...
\begin{frame}{De milhares de amostras a uma feature}
    ...
    \begin{tabular}{llrr}
        ...
        \midrule
        Bearing1\_1 & acc\_00001.csv & 0,000 & 0,56 \\
        Bearing1\_1 & acc\_00002.csv & 0,003 & 0,53 \\
        $\cdots$ & $\cdots$ & $\cdots$ & $\cdots$ \\
        Bearing1\_1 & acc\_02803.csv & 7,783 & 5,71 \\
        \bottomrule
    \end{tabular}
    \normalsize
\end{frame}
...
\begin{frame}{Passo 1--2: As \textit{features} traçam a degradação no tempo}
    \begin{columns}
        ...
        \end{itemize}
        \begin{alertblock}{Eixo Y é RMS de banda; a parada é por pico de 20\,g}
            \footnotesize Atenção ao eixo Y: ele mostra o \textbf{RMS de banda} (energia média, 10\,Hz--10\,kHz), mas o ensaio para quando a \textbf{amplitude de pico} do sinal cru ultrapassa \textbf{20\,g}. Como o RMS é sempre \textbf{menor que o pico} (e o fator de crista varia entre falhas), as curvas terminam \textbf{abaixo de 20\,g} e em níveis diferentes.
        \end{alertblock}
...
\begin{frame}{Passo 3: ajuste de distribuições de vida}
    ...
    \begin{block}{O que é verossimilhança e como entra aqui (MLE)}
        \scriptsize Cada par de parâmetros gera uma curva da distribuição. A \textbf{verossimilhança} $L(\theta)=\prod_{i=1}^{17} f(t_i;\theta)$ (produto das densidades nos dados) mede o quão \textbf{compatível} essa curva é com os 17 TTFs. O \textbf{MLE} acha automaticamente o $\theta$ ($\beta,\eta,\dots$) que \textbf{maximiza} $L$ --- a curva que ``melhor passa'' pelos dados. Na conta usa-se $\ln L=\sum_i\ln f(t_i;\theta)$ (vira soma e evita produtos $\sim\!10^{-40}$). \textbf{As três} distribuições são ajustadas assim, cada uma com seu próprio $L$ (o AICc compara depois). Sem ajuste manual: os parâmetros saem direto das 17 vidas.
    \end{block}
\end{frame}
